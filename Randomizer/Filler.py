...
def place_vanilla_items(world: TPWorld):
    if world.options.golden_bugs_shuffled.value == GoldenBugsShuffled.option_false:
        for bug_name, location in VANILLA_GOLDEN_BUG_LOCATIONS.items():
            ...
            world.pre_shuffle_pool.remove(bug_name)
    ...
    if world.options.poe_shuffled.value == PoeShuffled.option_false:
        ...
        for location in VANILLA_POE_LOCATIONS.items():
            assert (
                location in LOCATION_TABLE
            ), f"[Twilight Princess] {location=} not in location table"
    ...
    if (
        ...
        for location in VANILLA_SKY_CHARACTER_LOCATIONS.items():
            assert (
                location in LOCATION_TABLE
            ), f"[Twilight Princess] {location=} not in location table"
            ...
            world.pre_shuffle_pool.remove(sky_character_name)
...
def place_restricted_dungeon_items(world: TPWorld):
    ...
    # Dont bother building out everything if none of the items are shuffled (not vanilla)
    if not any(
        world.options.small_key_settings.value == SmallKeySettings.in_dungeon_shuffled
        or world.options.big_key_settings.value == BigKeySettings.in_dungeon_shuffled
        or (
            world.options.map_and_compass_settings.value
            == MapAndCompassSettings.in_dungeon_shuffled
        )
    ):
        return
    ...
    def _place_own_dungeon(
        ...
        for dungeon_name, data in vanilla.items():
            ...
            if dungeon_name == "Palace of Twilight":
                ...
                if (
                    world.options.palace_requirements
            ...
            for item in items_copy:
                world.pre_shuffle_pool.remove(item.name)
    ...
    def _place_any_dungeon(
        ...
        for dungeon_name in skipped_dungeons:
            ...
            for item_name, item_location_names in vanilla[dungeon_name]:
                assert (
                    item_name in ITEM_TABLE
    ...
    if world.options.small_key_settings.value == SmallKeySettings.option_own_dungeon:
        _place_own_dungeon(collection_state_small_key, VANILLA_SMALL_KEYS_LOCATIONS)
    if world.options.big_key_settings.value == BigKeySettings.option_own_dungeon:
        _place_own_dungeon(collection_state_big_key, VANILLA_BIG_KEY_LOCATIONS)
    if (
        world.options.map_and_compass_settings.value
        == MapAndCompassSettings.option_own_dungeon
    ):
        _place_own_dungeon(
            collection_state_map_and_compass, VANILLA_MAP_AND_COMPASS_LOCATIONS
        )

    if world.options.small_key_settings.value == SmallKeySettings.option_any_dungeon:
        _place_any_dungeon(collection_state_small_key, VANILLA_SMALL_KEYS_LOCATIONS)
    if world.options.big_key_settings.value == BigKeySettings.option_any_dungeon:
        _place_any_dungeon(collection_state_big_key, VANILLA_BIG_KEY_LOCATIONS)
    if (
        world.options.map_and_compass_settings.value
